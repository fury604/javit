 ...
 */
  ...
  private PooledThread startThread(Runnable target, String name) {
    ...
    synchronized (mPool) {
      ...
      // current thread gets its priority from the current Thread group
      // this has been overridden to be MAX_PRIORITY = 4
  ...
  void threadAvailable(PooledThread thread) {
    synchronized (mPool) {
      ...
      synchronized (mListeners) {
        for (Iterator it = mListeners.iterator(); it.hasNext();) {
          ((ThreadPoolListener)it.next()).threadFinished(event);
        }
      }
      mPool.notify();
    }
  ...
  private class PooledThread extends Thread {
    ...
    public PooledThread(String name) {
      ...
      mOriginalName = name;
      setPriority(1);  // added for race condition
    }
